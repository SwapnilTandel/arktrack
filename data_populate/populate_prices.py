        ...
        async with pool.acquire() as connection:
            async with aiohttp.ClientSession() as session:
                async with session.get(url=url) as response:
                    ...
                    params = [(stock_id, datetime.datetime.fromtimestamp(bar['t'] / 1000.0), round(bar['o'], 2),
...
async def get_stocks():
    ...
    async with pool.acquire() as connection:
        ...
        symbol_urls = {}
        for stock in stocks:
            symbol_urls[stock['id']] = f"https://api.polygon.io/v2/aggs/ticker/{stock['symbol']}" \
                                       f"/range/1/minute/2022-02-18/2022-02-18?apiKey={config.POLY_API_KEY}"
